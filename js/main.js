...
function createSongNameTooltip() {
  $tooltip.songName = document.createElement('DIV');
  $tooltip.songName.classList.add('song-name-tooltip');
  $tooltip.songName.textContent = 'song name tooltip';
  document.body.appendChild($tooltip.songName);
}

function showSongTooltip(text, e) {
  $tooltip.songName.textContent = text;
  var left = e.clientX + 5 + 'px';
  var top = e.clientY - 10 + 'px';
  $tooltip.songName.style.left = left;
  $tooltip.songName.style.top = top;
  if (!Array.from($tooltip.songName.classList).includes('show')) {
    $tooltip.songName.classList.add('show');
  }
}

function hideSongTooltip() {
  $tooltip.songName.classList.remove('show');
...
function playMedia() {
  ...
  if ($media.src !== window.location.href && $media.src) {
    ...
    // bufferSource.start($media.currentTime);
  } else {
    showNotification(
      'warning',
...
function setupEcho() {
  ...
  $media.currentTime = audioContext.currentTime;
...
function enableSettingPanelMove() {
  ...
  settingPanelHeader.addEventListener(
    ...
    function (e) {
      isMouseDown = true;
      offset = [$domElement.settingPanel.offsetLeft - e.clientX, $domElement.settingPanel.offsetTop - e.clientY];
    },
    true,
  );
}
...
function initTooltips() {
  ...
  $tooltip.songListAutoCloseTime.textContent =
    $appSetting.songListAutoCloseTime / 1000 + 's after mouse move in and out the list';
